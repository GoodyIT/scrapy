    ...
    def parse(self, response):
        ...
        for store in stores:
            try:
                ...
                item['address'] = store['AddressLine1'].encode('utf8').replace('\xc3\xad', 'i')
                item['address2'] = store['AddressLine2']

                if item['address2'].strip() != '' and item['address2'][0].isdigit() == True:
                    temp = item['address']
                    item['address'] = item['address2']
                    item['address2'] = temp

                # if item['address2'].find('Camino Real de La Plata 100') != -1:
                #     pdb.set_trace()

                item['phone_number'] = store['Phone'].replace(' ', '').replace('ext.', ' / ').replace('+', '')
                item['latitude'] = store['Latitude']
                item['longitude'] = store['Longitude']
                item['city'] = store['City'].encode('utf8').replace('\xc3\xb3', 'o')
